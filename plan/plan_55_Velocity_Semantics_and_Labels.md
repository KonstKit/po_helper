...
- **Action**: Lock velocity to sprint-based backend contract semantics.
...
- **Notes (recommended option B)**: date-range impact on velocity is implemented via deterministic `DateRangeOption -> sprints_count` mapping, passed to the same velocity endpoint.
...
- **Notes**: `sprints_count` mapping must be exported by `dashboardContract.ts` and reused by Dashboard, API client, and tests.
...
| --- | --- | --- | --- | --- |
| Sprint velocity with mapped window | `/v1/analytics/projects/{projectId}/velocity?sprints_count=N` | hours or points | `Sprint N` | "no sprint data" |
...
- `frontend/src/services/api/analytics.ts`
  ...
  - Modification Content: stable bucket rules + empty-state behavior + `sprints_count` request parameter support
...
- `frontend/src/pages/dashboard/dashboardContract.ts`
  - Modification Location: date-range to velocity mapping
  - Modification Content: canonical `VELOCITY_SPRINTS_COUNT_MAP`
  - Modification Reason: keep velocity window behavior deterministic and shared
...
- Date-range changes modify velocity through deterministic `sprints_count` mapping only (no fallback task-window model).
...
- Velocity request cache keys include `projectId` and mapped `sprints_count`.
