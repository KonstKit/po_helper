    ...
    for row in rows:
        ...
        try:
            ...
            if decrypted_value != original_value:
                ...
                logger.error(
                    f"     Decrypted length: {len(decrypted_value) if decrypted_value else 0}"
                )
                stats["errors"] += 1
                continue
            ...
            if not dry_run:
                ...
                logger.info(
                    f"  → ✓ ENCRYPTED: {original_length} chars → {len(encrypted_value)} chars"
                )
            else:
                logger.info(
                    f"  → ✓ WOULD ENCRYPT: {original_length} chars → {len(encrypted_value)} chars (DRY-RUN)"
                )
...
def print_summary(stats: dict, dry_run: bool) -> None:
    ...
    if stats["updated_rows"]:
        ...
        for row in stats["updated_rows"]:
            logger.info(
                f"  - {row['kind']} (id={row['id']}): {row['original_length']} → {row['encrypted_length']} chars"
            )
...
async def main():
    ...
    parser.add_argument(
        "--commit", action="store_true", help="Actually commit changes (default is dry-run)"
    )
