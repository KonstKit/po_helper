...
class JiraFieldMapper:
    ...
    def __init__(self, jira_service):
        ...
        self.field_cache = {}  # field_id -> field_info
        self.mappings = {}  # FieldType -> field_id
            ...
            for field in fields:
                ...
                if field_id:
                    self.field_cache[field_id] = {
                        'id': field_id,
                        'name': field.get('name', ''),
                        'type': field.get('schema', {}).get('type') if isinstance(field.get('schema'), dict) else None,
    ...
    async def _fetch_all_fields(self) -> List[Dict]:
        ...
        for version in [3, 2]:
            try:
                url = f"{self.jira.base_url}/rest/api/{version}/field"
                headers = self.jira._headers()
                response = self.jira._get(url, headers=headers, auth=self.jira.auth)
    ...
    async def _auto_map_fields(self):
        ...
        pattern_mappings = {
            FieldType.SPRINT: [
                r'sprint', r'iteration', r'cycle'
            ],
            FieldType.EPIC_LINK: [
                r'epic\s*link', r'epic', r'parent\s*link'
            ],
            FieldType.STORY_POINTS: [
                r'story\s*point', r'point', r'estimate.*point', r'size'
            ],
            FieldType.BUSINESS_VALUE: [
                r'business\s*value', r'value', r'benefit', r'roi'
            ],
            FieldType.TEAM: [
                r'team', r'squad', r'group'
            ],
            FieldType.FIX_VERSION: [
                r'fix.*version', r'release', r'version'
            ]
        }
            ...
            for field_id, field_info in self.field_cache.items():
                ...
                for pattern in patterns:
                    if re.search(pattern, field_name, re.IGNORECASE):
                        ...
                        logger.info(f"Auto-mapped {field_type.value} to {field_id} ({field_info['name']})")
                        break
    ...
    async def _fetch_issue_with_all_fields(self, issue_key: str) -> Optional[Dict]:
        ...
        for version in [3, 2]:
            try:
                url = f"{self.jira.base_url}/rest/api/{version}/issue/{issue_key}"
                params = {'expand': 'names,schema'}
                headers = self.jira._headers()
                response = self.jira._get(url, params=params, headers=headers, auth=self.jira.auth)
                response.raise_for_status()
                return response.json()
            except Exception as e:
    ...
    async def calibrate(self, sample_project_key: str, sample_size: int = 10) -> Dict[str, str]:
        """
        ...
        # Get sample issues
        issues = self.jira.get_project_issues(sample_project_key, max_results=sample_size)
        if not issues:
            logger.warning("No issues found for calibration; falling back to name-based auto mapping")
    ...
    async def _get_sprints_from_agile_api(self, issue_key: str) -> List[Dict[str, Any]]:
        ...
        try:
            url = f"{self.jira.base_url}/rest/agile/1.0/issue/{issue_key}"
            headers = self.jira._headers()
            response = self.jira._get(url, headers=headers, auth=self.jira.auth)
            response.raise_for_status()
            data = response.json()
