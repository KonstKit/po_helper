...
def _evaluate_gate(line_cov: Optional[float], branch_cov: Optional[float], min_line: float, min_branch: Optional[float]) -> Dict[str, Any]:
    reasons = []
    ok = True
    if line_cov is None:
...
async def quality_gate_pr(
    ...
    if project_id is not None:
        ...
        if proj and getattr(proj, 'quality_thresholds', None):
            ...
            if q.get('min_branch') is not None:
                min_branch = float(q.get('min_branch'))

    res = await db.execute(select(CoverageReport).where(CoverageReport.pr_number == pr_number).order_by(CoverageReport.created_at.desc()))
    cov = res.scalars().first()
    if not cov:
        return {"pr_number": pr_number, "pass": False, "reasons": ["no_coverage_found"], "line_coverage": None, "branch_coverage": None}
    out = _evaluate_gate(cov.line_coverage, cov.branch_coverage, min_line, min_branch)
    return {"pr_number": pr_number, **out, "line_coverage": cov.line_coverage, "branch_coverage": cov.branch_coverage}


@router.post("/gates/evaluate")
async def quality_gate_eval(
    payload: Dict[str, Any],
    db: AsyncSession = Depends(get_db),
):
    pr_number = payload.get('pr_number')
    ...
    if not cov:
        return {"pass": False, "reasons": ["no_coverage_found"], "line_coverage": None, "branch_coverage": None}
    out = _evaluate_gate(cov.line_coverage, cov.branch_coverage, min_line, min_branch)
    return {**out, "line_coverage": cov.line_coverage, "branch_coverage": cov.branch_coverage}


async def update_github_status(pr_number: int, gate_result: Dict[str, Any], db: AsyncSession) -> Dict[str, Any]:
    ...
    try:
        r = requests.post(url, json=status, headers={
            "Authorization": f"Bearer {settings.GITHUB_API_TOKEN}",
            "Accept": "application/vnd.github+json",
            "User-Agent": "po-helper",
        }, timeout=15)
        ok = 200 <= r.status_code < 300
        return {"updated": ok, "status_code": r.status_code, "response": r.json() if r.headers.get('content-type','').startswith('application/json') else None}
    except Exception as e:
        return {"updated": False, "reason": str(e)}


@router.post("/gates/evaluate-and-update")
async def quality_gate_eval_and_update(
    payload: Dict[str, Any],
    db: AsyncSession = Depends(get_db),
):
    result = await quality_gate_eval(payload, db)
...
async def update_github_check_run(pr_number: int, gate_result: Dict[str, Any], db: AsyncSession) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            pass
        return {"updated": ok, "status_code": r.status_code, "response": data}
    except Exception as e:
        return {"updated": False, "reason": str(e)}


@router.post("/gates/evaluate-and-check")
async def quality_gate_eval_and_check(
    payload: Dict[str, Any],
    db: AsyncSession = Depends(get_db),
):
    result = await quality_gate_eval(payload, db)
