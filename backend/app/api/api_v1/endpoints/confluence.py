...
from app.services.confluence_prd_parser import (
    extract_requirements_from_html,
...
async def start_celery_sync(
    ...
    if not settings.CELERY_ENABLED:
        raise HTTPException(
            ...
            detail="Celery is not enabled on this deployment; use the inline sync endpoints.",
        )
...
async def extract_prd_requirements(
    ...
    with handle_api_error(operation="extract_prd_requirements", context={"page_id": page_id}):
        ...
        html = page.get("body", {}).get("storage", {}).get("value", "")
        requirements_result = extract_requirements_from_html(html, page_id)
