...
async def connect_confluence(
    ...
    with handle_api_error(operation="connect_confluence"):
        ...
        if save:
            result = await db.execute(select(IntegrationSetting).where(IntegrationSetting.kind == "confluence"))
            ...
            async with transactional_session(db):
                ...
                row.base_url = (confluence_service.base_url or base_url).rstrip('/') if (confluence_service.base_url or base_url) else None
...
async def search_cql(cql: str, limit: int = 50):
    with handle_api_error(operation="search_cql"):
        results = confluence_service.search_content(cql, limit)
        items = []
        for r in results:
            links = r.get('_links', {})
            items.append({
                ...
                'last_updated': (r.get('version') or {}).get('when') or (r.get('history') or {}).get('lastUpdated', {}).get('when'),
            })
...
@router.get("/pages")
async def list_pages(space: Optional[str] = None, q: Optional[str] = None, limit: int = 50, start: int = 0):
    with handle_api_error(operation="list_pages"):
        items = confluence_service.list_pages(space=space, q=q, limit=limit, start=start)
        return {"count": len(items), "results": items}
...
async def get_space_tree(space_key: str, limit: int = 100, max_pages: Optional[int] = None):
    with handle_api_error(operation="get_space_tree", context={"space_key": space_key}):
        tree = confluence_service.list_space_tree(space_key, page_limit=limit, max_pages=max_pages)
...
async def sync_confluence(
    ...
    try:
        while True:
            ...
            pages = confluence_service.list_pages(space=space, q=q, limit=limit, start=page_start)
            if not pages:
            ...
            logger.info(f"Batch {batch_number} complete - created: {batch_created}, updated: {batch_updated}")

            # If not full sync or reached the end of results
            if not full or len(pages) < limit:
                logger.info(f"Stopping sync - full: {full}, pages_in_batch: {len(pages)}, limit: {limit}")
                break

            # Move to next batch
            page_start += len(pages)

        logger.info(f"Sync complete - total synced: {total_synced}, created: {total_created}, updated: {total_updated}")
...
async def sync_confluence_sse(
    ...
    async def generate_events() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            first_batch = confluence_service.list_pages(space=space, q=q, limit=limit, start=0)
            if first_batch:
            ...
            while True:
                ...
                for idx, p in enumerate(pages, 1):
                    ...
                    if idx % 3 == 0 or idx == len(pages):
                        ...
                        if total_pages_estimate > 0:
                            item_progress = min(95, int(10 + (current_total / total_pages_estimate) * 85))
                        else:
                ...
                if not full or len(pages) < limit:
                    logger.info(f"Stopping sync. Full: {full}, Pages in batch: {len(pages)}, Limit: {limit}")
                    break

                page_start += len(pages)

            # Send completion event
            logger.info(f"Sync complete. Total: {total_synced}, Created: {total_created}, Updated: {total_updated}")
...
async def start_celery_sync(
    ...
    with handle_api_error(operation="start_celery_sync", status_code=500, exception_map={ImportError: 503}):
...
async def sync_subtree(page_id: str, limit: int = 50, db: AsyncSession = Depends(get_db)):
    try:
        ids = confluence_service.iter_subtree(page_id, limit=limit)
        created = 0
        updated = 0
        for pid in ids:
            full = confluence_service.get_page_by_id(
                pid,
                expand='body.storage,version,history,metadata.labels,space',
            )
            cid = full.get('id')
            ...
            ptype = full.get('type')
            space_key = ((full.get('space') or {}).get('key')) if isinstance(full.get('space'), dict) else None
            links = full.get('_links', {})
            url = f"{confluence_service.base_url}{links.get('webui','')}" if confluence_service.base_url else links.get('webui')
            version = (full.get('version') or {}).get('number')
            created_at = (full.get('history') or {}).get('createdDate')
            last_updated = (full.get('version') or {}).get('when') or (full.get('history') or {}).get('lastUpdated', {}).get('when')
            labels = (full.get('metadata') or {}).get('labels')
            html = (full.get('body') or {}).get('storage', {}).get('value')
...
async def list_local_pages(space: Optional[str] = None, q: Optional[str] = None, limit: int = 50, offset: int = 0, db: AsyncSession = Depends(get_db)):
...
async def extract_prd_requirements(page_id: str):
    with handle_api_error(operation="extract_prd_requirements", context={"page_id": page_id}):
        ...
        for table in tables:
            ...
            if not headers:
                continue
            if ('Requirement' in headers or 'ID' in headers) or any('require' in h.lower() for h in headers):
...
async def extract_research(page_id: str):
    with handle_api_error(operation="extract_research", context={"page_id": page_id}):
        ...
        def cat(t: str) -> str:
            tl = t.lower()
            if any(k in tl for k in ["love", "like", "good", "great", "works well", "+", "рџ‘Ќ", "рџЉ"]):
                return 'positive'
            if any(k in tl for k in ["bug", "issue", "problem", "error", "pain", "hard", "confusing", "slow"]):
                return 'pain_points'
            if any(k in tl for k in ["feature", "request", "would like", "should have", "wishlist"]):
                return 'feature_requests'
            if any(k in tl for k in ["usability", "ux", "confusing", "difficult", "complex"]):
                return 'usability_issues'
            return 'other'

        grouped: dict[str, list[str]] = {k: [] for k in ['positive', 'pain_points', 'feature_requests', 'usability_issues', 'other']}
        for b in bullets:
