...
class ConfluenceSyncTask(Task):
    ...
    def update_progress(self, message: str, percent: int = None):
        ...
        if percent is None:
            percent = int((self.processed_pages / max(self.total_pages, 1)) * 100)
...
def sync_confluence_space(
    ...
    try:
        ...
        result = loop.run_until_complete(
            _async_sync_space(self, space_key, query, full_sync)
        )
        loop.close()
    ...
    except confluence_service.ConfluenceAPIError as exc:
    ...
    async with AsyncSessionLocal() as db:
        ...
        if pages:
            ...
            task.update_progress(
                f"Found approximately {task.total_pages} pages to sync",
                10
            )
        ...
        while pages:
            ...
            task.update_progress(
                f"Processing batch {batch_number} ({len(pages)} pages)..."
            )
            ...
            for idx, page_summary in enumerate(pages, 1):
                ...
                if idx % 5 == 0:
                    task.update_progress(
                        f"Processing: {full_page.get('title', 'Untitled')}"
                    )
...
async def _process_page(db, page_data: Dict[str, Any]) -> bool:
    ...
    space_key = (page_data.get('space', {}).get('key')
                 if isinstance(page_data.get('space'), dict) else None)
    ...
    if existing:
        ...
        existing.updated = _parse_dt(
            (page_data.get('version') or {}).get('when') or
            (page_data.get('history') or {}).get('lastUpdated', {}).get('when')
        )
